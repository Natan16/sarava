...
workon sarava  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development sarava npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop sarava
    docker rm sarava
    docker run --name sarava -d --env-file /home/ubuntu/sarava.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/sarava:/dkdata \
        sarava start_web.sh
}
...
echo_green "Welcome to sarava's dev env"
